...
# n_classes = len(score_board)

# m = max((n_classes+ 1) // 2, 2)
# n_plot_cols = 2

# fig, cells = plt.subplots(m, n_plot_cols, figsize=(n_plot_cols*8, m*8))
# for i,class_id in enumerate(score_board.keys()):
#   print('getting pr for class', class_id, i)
#   precision,recall = score_board[class_id].get_precision_recall()
#   print('after gettin pre,rec outside of class len = ', len(precision),len(recall), ' for class id ', class_id)
#   j = i%2
#   print('len recall ', len(recall))
#   cells[i, j].plot(recall, precision,
#                     color='blue', linewidth=1.0)
#   cells[i, j].set_xlabel('recall', fontsize=14)
#   cells[i, j].set_ylabel('precision', fontsize=14)
#   cells[i, j].grid(True)
#   cells[i, j].set_xticks(np.linspace(0, 1, 10))
#   cells[i, j].set_yticks(np.linspace(0, 1, 10))
#   cells[i, j].set_title("{}, AP: {:.3f}".format(
#       n_plot_cols*i+j+1, np.array(precision).mean(axis=0)), fontsize=16)
